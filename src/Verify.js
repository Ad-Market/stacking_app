...
export default function Verify() {
  ...
  const reset = () => {
    ...
    if (pass !== "" && cpass !== "" && pass == cpass) {
      axios
        .post(process.env.REACT_APP_SLAMBACKEND + "api/verify", send)
        .then((res) => {
  ...
  return (
    <div className="login-background">
      ...
      <div className="auth-form login-form">
        ...
        {success && (
          <div className="passChanged ft-20">
            <p className="text-white">{t("PasswordSaved")}</p>
          </div>
        )}
        {failure && <p style={{ color: "red" }}>{t("Fail")}</p>}
        {match && (
          <p style={{ color: "red", marginBottom: "0px" }}>
            {t("PasswordNotMatched")}
          </p>
        )}
        {!success && (
        ...
        {!success && (
          <div>
            <input
              type="password"
              className="verify_cpass slam-login-pass"
              id="pass"
        ...
        <div className="slam-confirm slam-verify">
          <p className="">
            <a className="slam-register-link" href="/">
              {t("Login")}
            </a>
          </p>
        </div>
