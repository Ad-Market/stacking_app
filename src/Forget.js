...
export default function Forget() {
    ...
    const send = () => {
        ...
        if(forget!=="" && email===false){
            axios.post(process.env.REACT_APP_SLAMBACKEND+'api/forget', forget_email)
            .then(res => {
            ...
            <div className="auth-form login-form">
                ...
                {box &&
                    <div className="forgetEmail">
                        <input type="email" className="slam-login-email" id="email" placeholder={t('AuthRecover')}  onChange={e=>{setForget(e.target.value); setEmail(false); setEmailNotExist(false);}}/>
                    </div>
                }
                {success && 
                ...
                {input && <p style={{color:'red', marginTop:'5px', textAlign:'left', marginBottom:'0px'}}>{t('FormFieldRequired')}</p>}
                {email && <p style={{color:'red', marginTop:'5px', textAlign:'left', marginBottom:'0px'}}>{t('RequireValidEmail')}</p>}
                {emailNotExist && <p style={{color:'red', marginTop:'5px', marginBottom:'0px'}}>{t('NotRegEmail')}</p>}
                
                {!success && <button className="btn slam-auth-button slam-forget-button" onClick={send}>{t('Send')}</button>}
                <div className="slam-confirm">
                    <p className="">{t('NoAccount')}? <a className="slam-register-link" href="/register">{t('Registration')}</a></p>
                </div>
            </div>
