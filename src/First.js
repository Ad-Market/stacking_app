...
function First() {
  ...
  const login = () => {
    ...
    if (email !== "" && password !== "" && validEamil === false && termsCond) {
      axios
        .post(process.env.REACT_APP_SLAMBACKEND + "api/signin", {
          email: email,
          password: password,
        })
        .then((res) => {
  ...
  const emailCodeSend = () => {
    ...
    if (emailCode) {
      axios
        .post(process.env.REACT_APP_SLAMBACKEND + "api/email-verify", send)
        .then((res) => {
          if (res.data.status === "success") {
            setSuccess(true);
            setFailure(false);
      ...
      <div className="auth-form login-form">
        ...
        {emailVerify && (
          <h4 className="text-danger">
            {t("VerificationLink")} <br />
            {t("VerifyEmail")}
          </h4>
        )}
        {verifyStatus == "emailVerified" && (
          <h4 className="text-success">{t("Verified")}</h4>
        )}
        {verifyStatus == "emailNotVerified" && (
          <h4 className="text-danger">{t("NotVerified")}</h4>
        )}
        {!basicLogin ? (
          <div id="basic_login">
            ...
            <div className="text-left slam-password-forget">
              <p
                className="slam-register-link"
                onClick={(e) => {
                  forget();
                }}
              >
            ...
            <div className="text-left slam-terms-condition">
              ...
              <label htmlFor="rememberMe">
                ...
                <a
                  ...
                  target="_blank"
                >
                  <strong>{t("Terms")}</strong> {t("And")}{" "}
                  <strong>{t("Conditons")}</strong>
                </a>
              </label>
            </div>
            <button
              className={
                ...
                validEamil === false &&
                termsCond
                  ? "btn slam-auth-button"
                  : "btn slam-auth-button disabled"
              }
              onClick={login}
              style={{ backgroundColor: formFill ? "#d8d9fe" : "" }}
            >
              {t("Login")}
            </button>
          </div>
          ...
          <div id="emailVerification">
            ...
            <p className="text-center text-white enterValidationCode">
              {t("Enter")} {t("Your")} {t("ValidationCode")}
            </p>
            {!success && (
              <div>
                <input
                  type="text"
                  className="verify_npass slam-login-email"
                  id="code"
                  placeholder={t("Code")}
                  onChange={(e) => {
            ...
            {!success && (
              <button
                className="btn slam-auth-button forget_button"
                onClick={emailCodeSend}
              >
                {t("Submit")}
              </button>
            )}
        ...
        <div className="slam-confirm">
          ...
          <p className="">
            {t("NoAccount")}?{" "}
            <span className="slam-register-link" onClick={register}>
              {t("Registration")}
            </span>
          </p>
        </div>
