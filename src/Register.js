...
export default function Register(){
    ...
    const register = () => {
        ...
        if(phone!=="" && email!=="" && pass!=="" && cpass!=="" && emailV===false && cpass === pass && termsCond){
        ...
        <div className="login-background">
            ...
            <div className="auth-form login-form">
                ...
                <div className="slamLogo">
                    <img src="/image/slam.png" className="slamcoin" alt="" onClick={login}/>
                </div>
                {success &&  <div className="mt-150 mb-150"><p className="success ft-20">{t('EmailConfirm')}</p></div>}
                ...
                {!success && <div className="">
                    ...
                    {phoneW &&  <p className="form-required">{t('FormFieldRequired')}</p>}
                    <div>
                    <input type="email" className="slam-register-pass" style={{marginTop:(phoneW)?'0px':''}} id="email" placeholder={t('EnterEmail')} onChange={e=>{setEmail(e.target.value); fillfunc(); setFail(false);}}/>
                    </div>
                    {emailW &&  <p className="form-required">{t('FormFieldRequired')}</p>}
                    {emailV &&  <p className="form-required">{t('InvalidEmail')}</p>}
                    <div>
                    <input type="password" className="slam-login-pass" style={{marginTop:(emailW||emailV)?'0px':''}} id="pass" placeholder={t('EnterPassword')} onChange={e=>{setPass(e.target.value); fillfunc();}}/>
                    </div>
                    {passW &&  <p className="form-required">{t('FormFieldRequired')}</p>}
                    <div>
                    <input type="password" className="slam-login-pass" style={{marginTop:(passW)?'0px':''}} id="cpass" placeholder={t('EnterConfirmPassword')} onChange={e=>{setCpass(e.target.value); fillfunc();}}/>
                    </div>
                    {cpassW &&  <p className="form-required">{t('FormFieldRequired')}</p>}
                    ...
                    <div className="slam-terms-condition row">
                        ...
                        <label htmlFor="rememberMe"><a className="text-white" href="https://slamcoin.io/SLM_Terms&Conditions.pdf" target="_blank">{t('AgreeTAC')}</a></label>
                    </div>
                    {(!termsCondW) &&  <p className="form-required">{t('ReqTAC')}</p>}

                    <button className={(phone!==""&&email!==""&&pass!==""&&cpass!==""&&emailV===false&& cpass === pass && termsCond) ? "btn slam-register-button" : "btn slam-register-button disabled"} 
                    style={{marginTop:(cpassW)?'25px':'', backgroundColor:fill?'#d8d9fe':''}} onClick={register}>{t('SignUp')}</button>
                </div>}
                <p className="slam-login-back" onClick={login}>{t('Login')}?</p>

            </div>
            <div className="slam-footer">
                <img src="/image/slamblack.png" className="slam-footer-icon" alt="" onClick={login}/>
